...
public class RunIPorODPIP
{
	 ...
	 */	
	public void run( boolean readCoalitionValuesFromFile, SolverNames solverName, boolean ipUsesLocalBranchAndBound, boolean ipSearchesMultipleSubspacesSimultaneously )
	{
		//Initialize the input parameters
		Input input = initializeInput( solverName );
		input.solverName = solverName;
		input.readCoalitionValuesFromFile = readCoalitionValuesFromFile;
		input.odpipUsesLocalBranchAndBound = ipUsesLocalBranchAndBound;
		input.odpipSearchesMultipleSubspacesSimultaneiously = ipSearchesMultipleSubspacesSimultaneously;
		
		//Create the main output folder
		Calendar calendar = Calendar.getInstance();
		SimpleDateFormat simpleDateFormat = new SimpleDateFormat( "yyyy-MM-dd (HH-mm-ss)" );
		String mainOutputFolder="";
		if( solverName == SolverNames.ODPIP ){
			if(( ipSearchesMultipleSubspacesSimultaneously == false )&&( ipUsesLocalBranchAndBound == true ))
				mainOutputFolder = "D:/CSGresults/ODP_IP_WithoutSimultanseousSearch "+simpleDateFormat.format(calendar.getTime());
			else
			if(( ipSearchesMultipleSubspacesSimultaneously == true )&&( ipUsesLocalBranchAndBound == false ))
				mainOutputFolder = "D:/CSGresults/ODP_IP_WithoutImprovedBranchAndBound "+simpleDateFormat.format(calendar.getTime());
			else
				mainOutputFolder = "D:/CSGresults/ODP_IP "+simpleDateFormat.format(calendar.getTime());
		}
		if( solverName == SolverNames.IP ){
			mainOutputFolder = "D:/CSGresults/IP "+simpleDateFormat.format(calendar.getTime());
		}
		if( solverName == SolverNames.ODPinParallelWithIP ){
			mainOutputFolder = "D:/CSGresults/ODPinParallelWithIP "+simpleDateFormat.format(calendar.getTime());
		}
		General.createFolder( mainOutputFolder );
		
		//The numbers of agents for which we are going to run our experiments
		int minNumOfAgents = 26; int maxNumOfAgents = 26;
	 ...
	 */
	private Input initializeInput( SolverNames solverName )
	{
		//Initialization
	...
	private void updateStringsOfResults( Input input, Result result, int distributionID )
	{
		...
		int k=0;
		if( result.ipIntegerPartitionGraph != null ){
