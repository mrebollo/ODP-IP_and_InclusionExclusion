...
public class MainFrame extends JFrame
{
	...
	JRadioButton runODPinParallelWithIP_radioButton = new JRadioButton();
	...
	JCheckBox ipUsesLocalBranchAndBound_checkBox = new JCheckBox();
	JCheckBox ipSearchesMultipleSubspacesSimultaneously_checkBox = new JCheckBox();
	...
	public Input readInputFromGUI()
	{
		...
		if( input.numOfAgents > 26 ){
			JOptionPane.showMessageDialog(null,	"The number of coalition structures cannot be handled by java for more than 26 agents!","Alert", JOptionPane.ERROR_MESSAGE);
		...
		else if (gamma_radioButton.isSelected()) input.valueDistribution = ValueDistribution.GAMMA;
		...
		else if( runInclusionExclusion_radioButton.isSelected()) input.solverName = SolverNames.InclusionExclusion;
		else if( runODPinParallelWithIP_radioButton.isSelected()) input.solverName = SolverNames.ODPinParallelWithIP;

		if( input.solverName == SolverNames.ODPIP ){
			if( ipUsesLocalBranchAndBound_checkBox.isSelected() )
				input.odpipUsesLocalBranchAndBound = true;
			else
				input.odpipUsesLocalBranchAndBound = false;

			if( ipSearchesMultipleSubspacesSimultaneously_checkBox.isSelected() )
				input.odpipSearchesMultipleSubspacesSimultaneiously = true;
			else
				input.odpipSearchesMultipleSubspacesSimultaneiously = false;
		}
		// Printing options
	...
	{
		// if running Inclusion-Exclusion [Bjorklund et al, 2009]
		if ( input.solverName == SolverNames.InclusionExclusion ){
			textArea.append("----------------------------------------------------\n    Inclusion-Exclusion ("+input.numOfAgents+" agents, "+ValueDistribution.toString(input.valueDistribution)+" distribution)\n----------------------------------------------------\n");
			if( input.numOfRunningTimes == 1 ){
				textArea.append("\nThe run time of Inclusion-Exclusion (in milliseconds):\n"+result.inclusionExclusionTime+"\n");
				textArea.append("\nThe best coalition structure found by Inclusion-Exclusion is:\n"+General.convertArrayToString(result.inclusionExclusionBestCSFound)+"\n");
				textArea.append("\nThe value of this coalition structure is:\n"+result.inclusionExclusionValueOfBestCSFound+"\n");
			}else
				textArea.append("\nThe average run time of Inclusion-Exclusion (in milliseconds):\n"+result.inclusionExclusionTime+" +/- "+result.inclusionExclusionTime_confidenceInterval+"\n");
		}
	...
	private void jbInit() throws Exception
	{
		...
		jPanel7.setBounds(new Rectangle(650, 415, 290, 265));
		...
		runDP_radioButton.addMouseListener(new MainFrame_runDP_radioButton_mouseAdapter(this));
		...
		runIDP_radioButton.addMouseListener(new MainFrame_runIDP_radioButton_mouseAdapter(this));
		...
		runODP_radioButton.setText("Run ODP [Rahwan et al., 2015]");
		runODP_radioButton.addMouseListener(new MainFrame_runODP_radioButton_mouseAdapter(this));
		...
		runIP_radioButton.addMouseListener(new MainFrame_runIP_radioButton_mouseAdapter(this));

		jPanel7.add(runODPinParallelWithIP_radioButton);
		radioButtonGroup2.add(runODPinParallelWithIP_radioButton);
		runODPinParallelWithIP_radioButton.setBounds(new Rectangle(6, 110, 270,22));
		runODPinParallelWithIP_radioButton.setText("Run ODP in parallel with IP");
		runODPinParallelWithIP_radioButton.setSelected(true);
		runODPinParallelWithIP_radioButton.addMouseListener(new MainFrame_runODPinParallelWithIP_radioButton_mouseAdapter(this));
		
		jPanel7.add(runODPIP_radioButton);
		radioButtonGroup2.add(runODPIP_radioButton);
		runODPIP_radioButton.setBounds(new Rectangle(6, 135, 270,22));
		runODPIP_radioButton.setText("Run ODP-IP [Rahwan et al., 2015]");
		runODPIP_radioButton.setSelected(true);
		runODPIP_radioButton.addMouseListener(new MainFrame_runODPIP_radioButton_mouseAdapter(this));
		
		jPanel7.add(ipUsesLocalBranchAndBound_checkBox);
		ipUsesLocalBranchAndBound_checkBox.setText("ODP improves the branch-and-bound of IP");
		ipUsesLocalBranchAndBound_checkBox.setBounds(new Rectangle(21, 160, 255, 22));
		ipUsesLocalBranchAndBound_checkBox.setSelected(true);

		jPanel7.add(ipSearchesMultipleSubspacesSimultaneously_checkBox);
		ipSearchesMultipleSubspacesSimultaneously_checkBox.setText("IP searches multiple subspaces simultaneously");
		ipSearchesMultipleSubspacesSimultaneously_checkBox.setBounds(new Rectangle(21, 185, 255, 22));
		ipSearchesMultipleSubspacesSimultaneously_checkBox.setSelected(true);
		
		jPanel7.add(runInclusionExclusion_radioButton);
		radioButtonGroup2.add(runInclusionExclusion_radioButton);
		runInclusionExclusion_radioButton.setBounds(new Rectangle(6, 210, 270,22));
		runInclusionExclusion_radioButton.setText("Run Inclusion-Exclusion [Bjorklund et al, 2009]");
		runInclusionExclusion_radioButton.addMouseListener(new MainFrame_runInclusionExclusion_radioButton_mouseAdapter(this));
		
		jPanel7.add(runCplex_radioButton);
		radioButtonGroup2.add(runCplex_radioButton);
		runCplex_radioButton.setBounds(new Rectangle(6, 235, 270,22));
		runCplex_radioButton.setText("Run ILOG's CPLEX");
		runCplex_radioButton.setEnabled(false);		
		runCplex_radioButton.addMouseListener(new MainFrame_runCplex_radioButton_mouseAdapter(this));
	...
	public void runODPIP_radioButton_mouseClicked(MouseEvent e)
	{
		ipUsesLocalBranchAndBound_checkBox.setEnabled(true);
		ipSearchesMultipleSubspacesSimultaneously_checkBox.setEnabled(true);
	}
	public void runODPinParallelWithIP_radioButton_mouseClicked(MouseEvent e)
	{
		ipUsesLocalBranchAndBound_checkBox.setEnabled(false);
		ipSearchesMultipleSubspacesSimultaneously_checkBox.setEnabled(false);
	}
	public void runIP_radioButton_mouseClicked(MouseEvent e)
	{
		ipUsesLocalBranchAndBound_checkBox.setEnabled(false);
		ipSearchesMultipleSubspacesSimultaneously_checkBox.setEnabled(false);
	}
	public void runDP_radioButton_mouseClicked(MouseEvent e)
	{
		ipUsesLocalBranchAndBound_checkBox.setEnabled(false);
		ipSearchesMultipleSubspacesSimultaneously_checkBox.setEnabled(false);
	}
	public void runIDP_radioButton_mouseClicked(MouseEvent e)
	{
		ipUsesLocalBranchAndBound_checkBox.setEnabled(false);
		ipSearchesMultipleSubspacesSimultaneously_checkBox.setEnabled(false);
	}
	public void runODP_radioButton_mouseClicked(MouseEvent e)
	{
		ipUsesLocalBranchAndBound_checkBox.setEnabled(false);
		ipSearchesMultipleSubspacesSimultaneously_checkBox.setEnabled(false);
	}
	public void runCplex_radioButton_mouseClicked(MouseEvent e)
	{
		ipUsesLocalBranchAndBound_checkBox.setEnabled(false);
		ipSearchesMultipleSubspacesSimultaneously_checkBox.setEnabled(false);
	}
	public void runInclusionExclusion_radioButton_mouseClicked(MouseEvent e)
	{
		ipUsesLocalBranchAndBound_checkBox.setEnabled(false);
		ipSearchesMultipleSubspacesSimultaneously_checkBox.setEnabled(false);
	}
	...
	public void solveCSGBasedOnPredefinedInput_radioButton_mouseClicked(MouseEvent e)
	{
		...
		runODPinParallelWithIP_radioButton.setEnabled(false);
		runCplex_radioButton.setEnabled(false);
		runInclusionExclusion_radioButton.setEnabled(false);
	...
	{
		if( runODPIP_radioButton.isSelected() ){
			ipUsesLocalBranchAndBound_checkBox.setEnabled(true);
			ipSearchesMultipleSubspacesSimultaneously_checkBox.setEnabled(true);
		}
		...
		runODPinParallelWithIP_radioButton.setEnabled(true);
		runInclusionExclusion_radioButton.setEnabled(true);
		//runCplex_radioButton.setEnabled(true); %runCplex is kept disabled, as the code has some mistakes
	}
...
class MainFrame_runODPIP_radioButton_mouseAdapter extends MouseAdapter {
	private MainFrame adaptee;
	MainFrame_runODPIP_radioButton_mouseAdapter(MainFrame adaptee) {
		this.adaptee = adaptee;
	}
	public void mouseClicked(MouseEvent e) {
		adaptee.runODPIP_radioButton_mouseClicked(e);
	}
}

class MainFrame_runODPinParallelWithIP_radioButton_mouseAdapter extends MouseAdapter {
	private MainFrame adaptee;
	MainFrame_runODPinParallelWithIP_radioButton_mouseAdapter(MainFrame adaptee) {
		this.adaptee = adaptee;
	}
	public void mouseClicked(MouseEvent e) {
		adaptee.runODPinParallelWithIP_radioButton_mouseClicked(e);
	}
}

class MainFrame_runIP_radioButton_mouseAdapter extends MouseAdapter {
	private MainFrame adaptee;
	MainFrame_runIP_radioButton_mouseAdapter(MainFrame adaptee) {
		this.adaptee = adaptee;
	}
	public void mouseClicked(MouseEvent e) {
		adaptee.runIP_radioButton_mouseClicked(e);
	}
}

class MainFrame_runDP_radioButton_mouseAdapter extends MouseAdapter {
	private MainFrame adaptee;
	MainFrame_runDP_radioButton_mouseAdapter(MainFrame adaptee) {
		this.adaptee = adaptee;
	}
	public void mouseClicked(MouseEvent e) {
		adaptee.runDP_radioButton_mouseClicked(e);
	}
}

class MainFrame_runIDP_radioButton_mouseAdapter extends MouseAdapter {
	private MainFrame adaptee;
	MainFrame_runIDP_radioButton_mouseAdapter(MainFrame adaptee) {
		this.adaptee = adaptee;
	}
	public void mouseClicked(MouseEvent e) {
		adaptee.runIDP_radioButton_mouseClicked(e);
	}
}

class MainFrame_runODP_radioButton_mouseAdapter extends MouseAdapter {
	private MainFrame adaptee;
	MainFrame_runODP_radioButton_mouseAdapter(MainFrame adaptee) {
		this.adaptee = adaptee;
	}
	public void mouseClicked(MouseEvent e) {
		adaptee.runODP_radioButton_mouseClicked(e);
	}
}

class MainFrame_runCplex_radioButton_mouseAdapter extends MouseAdapter {
	private MainFrame adaptee;
	MainFrame_runCplex_radioButton_mouseAdapter(MainFrame adaptee) {
		this.adaptee = adaptee;
	}
	public void mouseClicked(MouseEvent e) {
		adaptee.runCplex_radioButton_mouseClicked(e);
	}
}

class MainFrame_runInclusionExclusion_radioButton_mouseAdapter extends MouseAdapter {
	private MainFrame adaptee;
	MainFrame_runInclusionExclusion_radioButton_mouseAdapter(MainFrame adaptee) {
		this.adaptee = adaptee;
	}
	public void mouseClicked(MouseEvent e) {
		adaptee.runInclusionExclusion_radioButton_mouseClicked(e);
	}
}