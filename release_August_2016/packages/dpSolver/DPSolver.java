...
public class DPSolver {
	...
	public void runODP()
    {
    	...
    	if( input.solverName != SolverNames.ODPinParallelWithIP ){ //the the solver name must be ODP ( ODP-IP, because in ODP-IP, we actually run IDP, not ODP)
    		ODPisHelpingIP = false;
    		f = new double[ input.coalitionValues.length ];
    		for(int i=0; i<f.length; i++)
    			set_f(i, input.getCoalitionValue(i)); //Initialize every f(C) to be equal to v(C)
    	}else
    		ODPisHelpingIP = true;
    	...
    	int grandCoalition = (1 << n) - 1; // the set of agents, each "true" bit in A represents an agent.
        ...
        if(( ODPisHelpingIP )&&( getStop() )){ result.set_dpHasFinished(true);  clear_f();  return; }
    ...
    {    	
        ...
        for (int curSize = 2; curSize <= numOfAgents; curSize++)//For every size
        {
    		...
    		//If the coalition happens to be the grand coalition, then deal with this case separately
    ...
    private void evaluateSplitsOptimally( int C, int sizeOfC, int grandCoalition, int numOfAgents )
    {
    	...
    	for (int C1 = C & C; /* C1>0 */; C1 = C1-1 & C)
    	{
    		...
    		value = input.getCoalitionValue( C1+1 ) + input.getCoalitionValue( C2+2 );
    		if( bestValue < value ) bestValue = value;

    		value = input.getCoalitionValue( C1+2 ) + input.getCoalitionValue(  C2+1 );
    		if( bestValue < value ) bestValue = value;

    		if( (C1 & acceptableMinAgents)!=0 ){
    			value = input.getCoalitionValue( C1 ) + get_f( C2+3 );
    			if( bestValue < value ) bestValue = value;
    		}
    		if( (C2 & acceptableMinAgents)!=0 ){
    			value = input.getCoalitionValue( C2 ) + get_f( C1+3 );
    			if( bestValue < value ) bestValue = value;
    		}
