...
public class Input
{
	...
	public final int maxAllowedCoalitionValue = Integer.MAX_VALUE; //this is usually Integer.MAX_VALUE, but when testing inclusion-exclusion, we set it to a small value, e.g., 5
	public final boolean ipUsesBranchAndBound = true; // only relevant when running IP or ODP-IP
	public final boolean ipPruneSubspaces = true; // only relevant when running IP or ODP-IP. Determines whether IP is allowed to prune entire subspaces based on their bounds.
	...
	public boolean odpipUsesLocalBranchAndBound; // when this is "false", the ODP-IP does not improve branch-and-bound using the technique called "local branch and bound"
	public boolean odpipSearchesMultipleSubspacesSimultaneiously; // when this is "false" and we are running ODP-IP, IP will not search multiple subspaces simultaneously
	...
	public void generateCoalitionValues()
	{
		...
		//scale the coalition values if necessary
		if( maxCoalitionValue > maxAllowedCoalitionValue )
			for(int c=coalitionValues.length-1; c>=0; c--) //for every coalition, c
